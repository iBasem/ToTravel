...
### ✅ WIZ-5 / 7 / 9 / 10 done (2026-07-05)
- **WIZ-5 (admin approval):** `AdminPackageManagement` now shows **Approve & publish** / **Reject (→draft)** actions on `pending` packages (and Unpublish on published), backed by the existing `updatePackageStatus` (admins bypass the WIZ-8 guard). Publish flow is now complete end-to-end: agency submits → `pending` → admin approves → `published`.
- **WIZ-7 (inclusions round-trip): resolved by WIZ-6.** EditPackage loads the DB `inclusions` into `additionalInclusions`; the RPC builder flattens structured + additional on save, so the flat list round-trips with no data loss. The category *grouping* isn't reconstructable because the schema stores a flat `text[]` — a schema limitation, not a bug (would need a jsonb inclusions column to preserve categories).
