    ...
    <div className="relative">
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          />
          {loading && (
            <Loader2 className="absolute top-1/2 -translate-y-1/2 h-4 w-4 animate-spin text-muted-foreground end-3" />
          )}
        </div>
      </div>
        ...
        <div className="absolute z-50 w-full mt-2 bg-background border border-border rounded-lg shadow-lg max-h-60 overflow-y-auto">
          {results.map((result) => (
            <button
              ...
              className="w-full p-3 hover:bg-muted flex items-center gap-3 text-start border-b last:border-b-0 transition-colors"
            >
