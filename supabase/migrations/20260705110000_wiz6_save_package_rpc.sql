-- WIZ-6: Atomic multi-table package upsert as a single Postgres function.
--
-- Replaces the non-atomic, duplicated JS save logic (useCreatePackage +
-- EditPackage's delete-then-insert with unchecked errors). The whole function
-- body runs in one transaction, so any failure rolls the entire save back.
--
-- Authorization is enforced inside the function (SECURITY DEFINER): the caller
-- must be a signed-in agency, and on update must own the package. Agencies
-- CANNOT set `featured` (forced false — WIZ-8) or publish directly: submitting
-- for review sets status='pending' for admin approval (owner decision
-- 2026-07-05); admins move it to 'published' via their existing update policy.
...
revoke execute on function public.save_package(uuid, jsonb, boolean) from public, anon;
grant execute on function public.save_package(uuid, jsonb, boolean) to authenticated;
