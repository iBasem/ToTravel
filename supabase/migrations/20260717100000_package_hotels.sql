-- Package stays: the accommodations an agency shows on a package ("Where You'll
-- Stay"). Authored entirely by the agency — name typed, one image uploaded to the
-- existing package-media bucket. Deliberately NOT sourced from a hotel API: every
-- candidate provider forbids storing the name+photo we would need (Google Places
-- bars pre-fetch/caching and expires photo names; LiteAPI/Nuitee ToS §3 bars
-- "storing, copying, or creating databases from" their data and scopes the licence
-- to enabling bookings; Foursquare bills photos from the first call; Amadeus
-- Self-Service returns no images; Mapbox returns none and is temporary-use only).
--
-- Following the tour-operator convention, the name carries its own hedge
-- ("Apricot Hotel or similar") and the public page states accommodations are
-- subject to change — these are illustrative, not a bookable inventory.
--
-- day_numbers maps a stay onto itineraries.day_number (e.g. {1,2,5,7}); it is a
-- plain int[] rather than a join table because the days are already denormalised
-- into the itinerary collection that save_package replaces wholesale.
...
create table if not exists public.package_hotels (
  ...
  kind text not null default 'hotel'
    check (kind in ('hotel','resort','lodge','guesthouse','camp','cruise','apartment')),
  star_rating int check (star_rating between 1 and 5),
  day_numbers int[] not null default '{}',
...
-- Stays render on public package pages.
create policy "Anyone can view package hotels" on public.package_hotels
  for select using (true);

-- Owning agency manages its package's stays.
create policy "Agencies manage own package hotels" on public.package_hotels
  for all
...
-- Admins manage all.
create policy "Admins manage all package hotels" on public.package_hotels
  for all
  using (public.has_role((select auth.uid()), 'admin'))
  with check (public.has_role((select auth.uid()), 'admin'));

create trigger update_package_hotels_updated_at
  before update on public.package_hotels
  for each row execute function public.update_updated_at_column();

-- save_package v6: sync stays like the other child collections. Body is v5
-- verbatim plus the package_hotels block; nothing else changes.
...
begin
  ...
  insert into public.package_hotels (package_id, name, name_ar, kind, star_rating, day_numbers, upgrade_available, image_path, display_order)
  select v_pid,
    e->>'name', nullif(e->>'name_ar',''),
